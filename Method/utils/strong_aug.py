...
class StrongAugmentations(nn.Module):
    def __init__(self):
        super(StrongAugmentations, self).__init__()

    def forward(self, tensor, num_x, num_y, shuffle_index=None):
        tensor = RandomBrightnessContrast(tensor)
        tensor = add_gaussian_noise(tensor, tensor.device)
        return tensor, None
