...
class ViGUNet(nn.Module):
    def __init__(self):
        ...
        self.Encoder = ViG()
        save_model = torch.load('../GraphGazer/models/pretrained_models/pvig_s_82.1.pth.tar')
        ...
        self.deep_sup_head_5 = nn.Sequential(
            nn.Conv2d(filters[3], 1, kernel_size=1, stride=1, padding=0))
        self.deep_sup_head_6 = nn.Sequential(
            nn.Conv2d(filters[2], 1, kernel_size=1, stride=1, padding=0))
        self.deep_sup_head_7 = nn.Sequential(
            nn.Conv2d(filters[1], 1, kernel_size=1, stride=1, padding=0))
        self.deep_sup_head_8 = nn.Sequential(
            nn.Conv2d(filters[0], 1, kernel_size=1, stride=1, padding=0))
    ...
    def forward(self, x):
        ...
        x1, x2, x3, x4, x5 = f_list[0], f_list[1], f_list[2], f_list[3], f_list[4]
        # Bridge
        ...
        x6 = self.upsample_1(x_bridge) # 640x14x14
        x6 = self.up_residual_conv1(torch.cat([x6, x4], dim=1) ) # 400x14x14 # 1040x14x14

        x7 = self.upsample_2(x6) # 400x28x28
        x7 = self.up_residual_conv2(torch.cat([x7, x3], dim=1)) # 160x28x28 # 560x28x28

        x8 = self.upsample_3(x7) # 160x56x56
        x8 = self.up_residual_conv3(torch.cat([x8, x2], dim=1) ) # 80x56x56 # 240x56x56


        # Deep Supervision
        out_seg_x2 = self.deep_sup_head_2(x2)
        out_seg_x3 = self.deep_sup_head_3(x3)
        out_seg_x4 = self.deep_sup_head_4(x4)
        out_seg_p = self.deep_sup_head_5(x_bridge)
        out_seg_x6 = self.deep_sup_head_6(x6)
        out_seg_x7 = self.deep_sup_head_7(x7)
        out_seg_x8 = self.deep_sup_head_8(x8)
        out_seg = self.segmentation_head(x8) # 1x224x224

        feature_list = [x_bridge, x6, x7, x8]
        output_list = [out_seg_p, out_seg_x6, out_seg_x7, out_seg_x8, out_seg]
...
if __name__ == "__main__":
    ...
    feature_list, output_list = model(inputs)
